...
public class DownloadImagesStartedService 
    ...
    public static Intent makeIntent(Context context,
        ...
        // the Intent.
    	Intent downloadImageStartedServiceIntent = new Intent(context, DownloadImagesStartedService.class);
    	downloadImageStartedServiceIntent.setData(url);
    	downloadImageStartedServiceIntent.putExtra(RequestMessage.REQUEST_CODE, requestCode);
    	downloadImageStartedServiceIntent.putExtra(RequestMessage.DIRECTORY_PATHNAME, directoryPathname);
    	Message message =RequestMessage.makeRequestMessage
    			(requestCode, url,directoryPathname, new Messenger(downloadHandler)).getMessage();
    	downloadImageStartedServiceIntent.putExtra(REQUEST_MESSAGE, message);
    	return downloadImageStartedServiceIntent;
    }
    ...
    public void onHandleIntent(Intent intent) {
    	...
    	final Uri uriImageToDownload = intent.getData();
    	final Uri dirPathName = (Uri) data.get(RequestMessage.DIRECTORY_PATHNAME);

        // Download the requested image.
        // TODO -- you fill in here.
    	Uri downloadedImage = NetUtils.downloadImage(
    	...
    	{
    		sendPath(messenger, downloadedImage, uriImageToDownload,Activity.RESULT_OK);
 
    	}
    	else
    	{
    		sendPath(messenger, downloadedImage, uriImageToDownload,Activity.RESULT_CANCELED);
