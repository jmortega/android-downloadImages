...
public class DownloadImagesBoundService extends LifecycleLoggingService {
	...
	public void handleMessage(Message message) {
	    ...
	    // TODO -- you fill in here.7
	    final Uri imageUrl = Uri.parse(requestMessage.getData().get(RequestMessage.IMAGE_URL).toString());
	    
	    // Get the directory pathname where the image will be
	    // stored.
	    // TODO -- you fill in here.
	    //final Uri dirPathname = requestMessage.getDirectoryPathname();
	    final Uri dirPathname = Uri.parse(requestMessage.getData().get(RequestMessage.DIRECTORY_PATHNAME).toString());
	    
	    // Get the requestCode for the operation that was invoked
	    // by the Activity.
	    // TODO -- you fill in here.
	    int reqCode = (int)requestMessage.getData().get(RequestMessage.REQUEST_CODE);
		...
		new Runnable() {
			...
			public void run() {
		    	...
		    	Uri downloadedImageUri = NetUtils.downloadImage(ctx, imageUrl, dirPathname);
	...
	public void sendPath(Messenger messenger, 
		...
		if(url !=null){
			replyMsg.setImageURL(url);
    	}
    	
    	if(pathToImageFile!=null){
    		replyMsg.setImagePathname(pathToImageFile);
    	}
    	
    	replyMsg.setRequestCode(requestCode);
